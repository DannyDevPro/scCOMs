    ...
    constructor(runtime) {
        ...
        this._formatMessage = runtime.getFormatMessage({
            'zh-cn': {
                ...
                'AusCOM.wsocket_create': 'ws.创建websocket,协议[yi],并返回id',
                ...
                'AusCOM.socket_create': 'sock.创建[yi]方式[method]并返回id 接收长度[buff]',
                'AusCOM.socket_recv': 'sock.收到数据id [id]',
                'AusCOM.socket_send': 'sock.发送数据id [id]数据[TEXT]',
                'AusCOM.socket_conclose': 'sock.断开连接id[id]',
                'AusCOM.socket_connect': 'sock.连接id[id] ip [ip]',
                'AusCOM.socket_breakev': 'sock.事件：断开连接id[id]',
            ...
            en: {
                ...
                'AusCOM. socket_ sreate ': 'sock. Create [yi] method [method] and return ID,buff[buff]',
                ...
                'AusCOM. socket_ connect': 'sock. Connection ID [id] IP [ip] ',
                'AusCOM.socket_breakev': 'sock.evemt:connection break off id[id]',
        ...
        this.sock = []
        window.AusCOM = { 'wsrecv_': [], 'wsnotalive_': [], 'json_list': [] }
        ...
        return {
            ...
            blocks: [
...

                {
                    opcode: 'help',
                    blockType: BlockType.REPORTER,
                    text: this.formatMessage('AusCOM.help'),
                },
            ],
            menus: {
                HTTP_method: [
            ...
            if (String(id) == this.sock[i]["id"]) {
                ...
                try {
                    ...
                    this.sock[i]['obj'].onmessage = function (evt) {
                        //alert(evt.data)
                        window.AusCOM.wsrecv_.push({ 'id': this.sock[i]['id'], 'nr': evt.data })
                    }
                    this.sock[i]['obj'].onclose = function (evt) {
                        window.AusCOM.wsnotalive_.push(this.sock[i]['id'])
                    }
                    return true
                }
                catch (error) {
        ...
        for (let i = 0, len = this.sock.length; i < len; i++) {
            if (String(id) == this.sock[i]["id"]) {
                for (let i = 0, len = window.AusCOM.wsrecv_.length; i < len; i++) {
                    if (String(id) == window.AusCOM.wsrecv_[i]["id"]) {
                        try {
                            if (window.AusCOM.wsrecv_[i]["obj"] == null) { return false } else { window.AusCOM.wsrecv_[i]["obj"].send(text); return true }
                        }
                        catch (error) {
                            return false
                        }
